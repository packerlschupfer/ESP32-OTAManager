...
void handleShutdown() {
    ...
    if (OTATask::taskHandle != nullptr) {
        ...
        if (otaHandle != nullptr && !taskManager.stopTask(otaHandle)) {
            LOG_WARN(LOG_TAG_MAIN, "Failed to stop OTATask");
        }
    }
    ...
    if (monitoringHandle != nullptr && !taskManager.stopTask(monitoringHandle)) {
    ...
    if (sensorHandle != nullptr && !taskManager.stopTask(sensorHandle)) {
        LOG_WARN(LOG_TAG_MAIN, "Failed to stop SensorTask");
    }
